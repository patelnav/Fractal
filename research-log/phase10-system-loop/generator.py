...
class HuggingFaceGenerator(Generator):
    ...
    def generate_batch(self, questions: List[str], n: int = 5) -> List[List[str]]:
        """
        Generate solutions for multiple questions in TRUE parallel using chat templates.
        """
        import torch
        ...
        # 2. Expand for n candidates
        expanded_input_ids = []
        for ids in input_ids_list:
            expanded_input_ids.extend([ids] * n)
            
        # 3. Pad (Left padding for generation)
        self.tokenizer.padding_side = 'left'
        if self.tokenizer.pad_token_id is None:
             self.tokenizer.pad_token_id = self.tokenizer.eos_token_id
             
        # Flip, pad right, flip back = pad left
        input_tensor = pad_sequence([t.flip(0) for t in expanded_input_ids], batch_first=True, padding_value=self.tokenizer.pad_token_id).flip(1).to(self.device)
        ...
        for i in range(total_seqs):
            ...
            candidates_flat.append(text.strip())
            
        # Reshape
        for i in range(batch_size):
            start = i * n
            end = start + n
            all_results.append(candidates_flat[start:end])
